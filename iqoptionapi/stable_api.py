...
class IQ_Option:
    ...
    def update_ACTIVES_OPCODE(self):
        ...
        self.get_ALL_Blitz_ACTIVES_OPCODE()
        # crypto /dorex/cfd
    ...
    def get_instruments(self, type):
        ...
        while self.api.instruments == None:
            try:
                ...
                start = time.time()
                while self.api.instruments == None and time.time() - start < 15:
                    pass
            except:
    ...
    def get_ALL_Blitz_ACTIVES_OPCODE(self):
        while True:
            init_info = self.get_all_init_v2()
            if init_info is not None:
                for i in init_info["blitz"]["actives"]:
                    OP_code.ACTIVES[str(init_info["blitz"]["actives"][i]["name"]).split(".")[1]] = int(i)
                break
            time.sleep(0.5)
        ...
        while attempts < max_attempts:
            ...
            while time.time() - start < timeout:
                if self.api.api_option_init_all_result is not None:
                    try:
                        if self.api.api_option_init_all_result.get("isSuccessful"):
                            return self.api.api_option_init_all_result
                    except KeyError:
                        logging.error("Resposta da API incompleta.")
                        break
                time.sleep(0.5)  # Evita uso excessivo de CPU

            logging.warning(f"**warning** get_all_init late {timeout} sec")
            attempts += 1

        logging.error("Número máximo de tentativas atingido. Finalizando.")
        return None

    
    def get_all_initv3(self):
    ...
    def __get_binary_open(self):
        ...
        if binary_data and binary_data is not None:
    ...
    def __get_blitz_open(self):
        ...
        instruments_type = "blitz"
        ins_data = self.get_all_init_v2()
        if ins_data is not None:
            try:
                if instruments_type in ins_data:
                    for actives_id in ins_data[instruments_type]["actives"]:
                        active=ins_data[instruments_type]["actives"][actives_id]
                        name = str(active["name"]).split(".")[1]
                        if active["enabled"] == True:
                            if active["is_suspended"] == True:
                            ...
                            self.OPEN_TIME[instruments_type][name]["open"] = active["enabled"]
            except:
                pass

    def code_actives(self):
        ins_data=self.get_all_init_v2()
        instrument_type='blitz'
        code_active=None
        if instrument_type in ins_data:
            for i in ins_data[instrument_type]['actives']:
                name=str(ins_data[instrument_type]["actives"][i]['name']).split('.')[1]

                for active in OP_code.ACTIVES:
                    OP_code.ACTIVES[active]=int(i)
                    break
    ...
    def get_all_open_time(self):
        ...
        blitz = threading.Thread(target=self.__get_blitz_open)
        other = threading.Thread(target=self.__get_other_open)

        binary.start(), blitz.start(),other.start()

        binary.join(), blitz.join(),other.join()
    ...
    def get_all_profit(self):
        ...
        instruments_type='blitz'
        if init_info is not None:
            if instruments_type in init_info:
                for actives_id in init_info[instruments_type]["actives"]:
                    active=init_info[instruments_type]["actives"][actives_id]
                    name = str(active["name"]).split(".")[1]
                    all_profit[name][instruments_type] = (
                    100.0 -
                    init_info[instruments_type]["actives"][actives_id]["option"]["profit"][
                        "commission"]) / 100.0
...
##############################################################################################
    ...
    def buy_blitz(self, price, payout, ACTIVE, ACTION, expirations):
	    """
	        Função para realizar uma compra com tratamento de erros e lógica de timeout.
	        
	        Parâmetros:
	        - price: Preço da compra.
	        - payout: Pagamento esperado.
	        - ACTIVE: Ativo da compra.
	        - ACTION: Tipo de ação (ex: compra/venda).
	        - expirations: Tempo de expiração.
	        - timeout: Tempo limite para resposta (em segundos).
	        Retorna:
	        - Resultado da API e ID da compra ou mensagem de erro.
	    """
	    timeout=3
	    self.api.buy_multi_option = {}
	    self.api.buy_successful = None
	    req_id = str(randint(0, 10000))  # Gera um ID único para a requisição
	    try:
	        self.api.buy_multi_option[req_id] = {"id": None}  # Inicializa o ID na estrutura
	    except KeyError:
	        logging.error("Erro ao inicializar buy_multi_option para req_id")
	        pass
	    print('Buy Blitz')  # Indicação visual para debug
	    self.api.buyv4(float(price), OP_code.ACTIVES[ACTIVE], str(ACTION), int(expirations), payout, req_id)
	    start_t = time.time()  # Marca o início do timeout
	    id = None
	    self.api.result = None
	    while time.time() - start_t < timeout:  # Checa se está dentro do tempo limite
	        try:
	            result = self.api.buy_multi_option.get(req_id, {})
	            if "message" in result:
	                return False, result["message"]  # Retorna mensagem de erro se disponível
	            id = result.get("id")  # Obtém o ID da compra
	            if id is not None:  # Se ID for encontrado, retorna o resultado
	                return self.api.result, id
	        except Exception as e:
	            logging.error(f"Erro durante processamento: {e}")
	    
	    # Se exceder o timeout, registra erro e retorna sem resultado
	    logging.error('**warning** compra excedeu 5 segundos')
	    return False, None
